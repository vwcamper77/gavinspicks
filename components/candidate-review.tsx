...
export default function CandidateReview({candidates}:{candidates:Candidate[]}){
 ...
 useEffect(()=>{fetch('/api/admin/curation',{cache:'no-store'}).then(async r=>{if(r.status===401){setSignedIn(false);return}if(!r.ok)throw Error();const d=await r.json();setDecisions(Object.fromEntries((d.decisions||[]).map((x:Decision)=>[x.url,x])));setSignedIn(true)}).catch(()=>setMessage('Could not load saved review decisions.'))},[]);
 ...
 async function decide(c:Candidate,action:Decision['action']){
  ...
  try{const r=await fetch('/api/admin/curation',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:c.url,action})});const d=await r.json();if(!r.ok)throw Error(d.error||'Could not save decision.');setDecisions(v=>({...v,[c.url]:d.decision}));}
  catch(e){setMessage(e instanceof Error?e.message:'Could not save decision.')}finally{setBusy(null)}
 }
 if(signedIn===false)return <section className="rounded-2xl border p-8"><h2 className="text-xl font-semibold">Admin sign-in required</h2><p className="mt-2 text-sm text-muted-foreground">Candidate decisions write to the private Gavin’s Picks store.</p><Link href="/admin" className="mt-5 inline-block rounded-md bg-foreground px-4 py-3 text-sm font-medium text-background">Sign in at Admin</Link></section>;
 return <>
  ...
  <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
   {pending.map((c,i)=>{const d=derive(c);const working=busy===c.url;return <article key={c.url} className="overflow-hidden rounded-2xl border bg-background shadow-sm">
