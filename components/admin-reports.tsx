...
export default function AdminReports({signedIn}:{signedIn:boolean}){
 ...
 return <main className="admin-workspace"><header className="admin-header"><div><a href="/">← Gavin’s Picks</a><h1>Availability reports</h1></div>{authenticated&&<button onClick={logout} disabled={busy}>Sign out</button>}</header>
 {error&&<p className="admin-error" role="alert">{error}</p>}{notice&&<p className="admin-notice" role="status">{notice}</p>}
 {!authenticated?<form className="admin-login" onSubmit={login}><h2>Owner sign-in</h2><p>Enter your private admin access key to review visitor reports.</p><label>Access key<input name="key" type="password" required autoComplete="current-password"/></label><button disabled={busy}>{busy?'Signing in…':'Sign in'}</button></form>:<>
